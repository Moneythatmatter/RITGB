...
const lines = [
  { text: "WEBSITES", isAccent: false },
  { text: "WORTH", isAccent: false },
  { text: "EXPLORING.", isAccent: true },
];
...
export default function WebsiteDesignHero() {
  ...
  return (
    ...
    >
      <div className="px-6 md:px-14 lg:px-20 py-12 md:py-20 grow flex flex-col items-center justify-center text-center relative z-10">
        <h1 className="flex flex-col items-center text-center select-none">
          {lines.map((item, index) => (
            <span key={index} className="overflow-hidden block">
              <span
                className={`web-line font-(family-name:--font-right-grotesk) text-[15vw] sm:text-[13vw] lg:text-[9.5vw] font-black leading-[0.88] uppercase tracking-[-0.03em] block ${item.isAccent ? "text-[#4db685]" : "text-white"
                  }`}
              >
                {item.text}
              </span>
        ...
        <div className="web-sub-content flex flex-col items-center text-center mt-6 md:mt-8 max-w-xl">
          <p className="text-neutral-300 text-base sm:text-lg lg:text-xl font-light leading-relaxed mb-6 md:mb-8">
            We design websites that show people who you are, what you offer, and
      ...
      <div className="w-full bg-white text-black py-4 md:py-5 overflow-hidden select-none border-y border-neutral-200 relative z-10">
