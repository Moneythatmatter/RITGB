...
const lines = [
  { text: "MAKE IT", isAccent: false },
  { text: "EASY TO", isAccent: false },
  { text: "LOVE USING.", isAccent: true },
];
        ...
        <h1 className="flex flex-col items-center text-center select-none">
          {lines.map((item, index) => (
            <span key={index} className="overflow-hidden block">
              <span
                className={`uiux-line font-(family-name:--font-right-grotesk) text-[15vw] sm:text-[13vw] lg:text-[9.5vw] font-black leading-[0.88] uppercase tracking-[-0.03em] block ${item.isAccent ? "text-[#4db685]" : "text-white"
                  }`}
              >
                {item.text}
              </span>
